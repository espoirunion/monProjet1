...
object CreateNaiveBayesModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf: SparkConf = new SparkConf()
         .setMaster("local")
         .setAppName("NaiveBayesModelCreation")
    ...
    val model = NaiveBayes.train(train, lambda = 0.1)  // création d'un modèle naive bayes et l'executer sur les données (train)
    ...
    model.save(sc, "C:/Users/Leila/IdeaProjects/project3_final/src/main/resources/MyModelNB1")
    ...
    val metrics = new MulticlassMetrics(predictionAndLabel)   //un autre critère de la précision
    println("accuracy = "+ accuracy + "and metrics = "+ metrics.weightedFMeasure)  //précision du modèle = 0.7912 et metrics = O.78
  }
}